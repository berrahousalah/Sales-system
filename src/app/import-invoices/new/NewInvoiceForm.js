...
import { createImportInvoice } from "../actions";

export default function NewImportInvoicePage({ suppliers }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Combobox already handles the hidden input because we pass `name="supplierId"`
    
    startTransition(async () => {
      const result = await createImportInvoice(formData);
      if (result.success) {
        router.push(`/import-invoices/${result.invoice.id}`);
      } else {
    ...
    <div className="min-h-screen bg-gray-50/50 p-6 md:p-8 lg:p-12">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">New Import Invoice</h1>
          ...
          <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
            ...
            <div>
              ...
              <p className="text-xs text-amber-600 font-medium mt-0.5">
                ⚠ Once saved, Supplier and Date become permanently immutable
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-2">
              <button
                ...
                className="flex items-center justify-center gap-2 w-full px-4 py-3 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium"
              >
